public interface Scheduler {
    ...
    public static final Scheduler DEFAULT = new Scheduler() {  //a default schedule.
        public void schedule(Configuration config) {
            new LogUpdate("(pre) ON CPU: " + config.cpu.current() + "/" + config.temp  +
                    " READY QUEUE: " + config.ready);  //log scheduling event.
            ...
            new LogUpdate("(post) Sched: ON CPU: " + config.cpu.current() + "/" + config.temp +
                    " READY QUEUE: " + config.ready);  //log scheduling event.
        }
    };
}
